...
const tabs = [
    ...
    visual: () => (
      <div className="grid grid-cols-4 gap-2 text-[10px]">
        {["Intake", "Plan", "Knowledge", "Tool", "Policy", "Respond", "Log", "Notify"].map((s) => (
          <div
            key={s}
            className="rounded-md border border-border bg-surface-muted p-2 text-center text-foreground"
          >
            {s}
          </div>
        ))}
  ...
  {
    id: "rag",
    label: "Enterprise Knowledge / RAG",
    icon: BookOpen,
    title: "Grounded answers, every time.",
    desc: "Connect PDFs, policies, SharePoint, ERP, CRM. Citations on every response.",
    visual: () => (
      <div className="space-y-1.5 text-xs">
        <div className="rounded-md border border-border bg-surface p-2 text-foreground">
          Answer: Policy 4.2 allows 12 days PTO carryover.
        </div>
        <div className="text-[10px] text-muted-foreground">
          Sources · HR-Policy-v4.pdf · Confluence/HR/PTO
        </div>
      </div>
    ),
  },
  {
    ...
    visual: () => (
      <div className="grid grid-cols-3 gap-2 text-[10px]">
        {["Slack.post", "SAP.read", "CRM.update", "Email.send", "Drive.read", "Ticket.create"].map(
          (t) => (
            <div
              key={t}
    ...
    visual: () => (
      <div className="space-y-2 text-xs">
        {[
          ...
          <div
            key={r.m}
            className="grid grid-cols-4 items-center rounded-md border border-border bg-surface p-2 text-foreground"
          >
            <span>{r.m}</span>
            <span className="text-muted-foreground">{r.c}</span>
            <span className="text-muted-foreground">{r.l}</span>
            <span className="text-[var(--cyan)]">{r.f}</span>
          </div>
        ))}
      </div>
    ),
  },
  {
    ...
    visual: () => (
      <div className="flex items-center justify-between rounded-md border border-border bg-surface p-3 text-xs">
        ...
        <span className="h-5 w-9 rounded-full bg-[var(--cyan)]/40 p-0.5">
          <span className="block h-4 w-4 translate-x-4 rounded-full bg-[var(--cyan)] transition-transform" />
        </span>
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-8">
            <div
              key={active}
              className="animate-[fade-up_0.4s_ease-out_both] rounded-2xl border border-border bg-surface p-8 shadow-elegant"
            >
              <h3 className="font-display text-2xl font-semibold text-foreground">
                {current.title}
              </h3>
              <p className="mt-2 text-muted-foreground">{current.desc}</p>
              <div className="mt-6 rounded-xl border border-border bg-surface-muted/60 p-5">
                <Visual />
              </div>
            </div>
          </div>
