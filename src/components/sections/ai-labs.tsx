...
const agents = [
  {
    ...
    desc: "Reviews workflows against internal policy and regulatory frameworks.",
  },
  {
    icon: Landmark,
    title: "Citizen Services Agent",
    desc: "Handles multilingual citizen queries with auditable, sourced answers.",
  },
  {
    icon: FileText,
    title: "Finance Document Review Agent",
    desc: "Extracts, classifies, and routes invoices, contracts, and statements.",
  },
  {
    icon: GraduationCap,
    title: "Education Support Agent",
    desc: "Helps faculty and students navigate policies and registration workflows.",
  },
];
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-muted-foreground">
              Try interactive examples of grounded, governed agents. Each card runs a short
              workflow: request → knowledge check → agent response → governance step → result.
            </p>
          </div>
          <a
            href="#cta"
            className="rounded-md border border-border bg-surface px-4 py-2.5 text-sm font-medium text-foreground hover:bg-surface-muted"
          >
            Launch AI Lab
          </a>
        </div>
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-1.5 text-[10px]">
                {["Request", "Knowledge", "Response", "Governance"].map((s) => (
                  <span
                    key={s}
                    className="rounded-full border border-border bg-surface px-2 py-0.5 text-foreground/80 shadow-sm"
                  >
                    {s}
                  </span>
                ))}
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {tools.map(({ icon: Icon, t }) => (
            <div
              key={t}
              className="flex items-center gap-3 rounded-xl border border-dashed border-border bg-surface p-4 shadow-sm"
            >
